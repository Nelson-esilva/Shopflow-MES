...
function LineSearchBar({ searchQuery, onSearch, onClear, placeholder }) {
  const [inputValue, setInputValue] = useState(searchQuery || '');
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));

  // Sincroniza o valor do input com a prop searchQuery
  useEffect(() => {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', width: isMobile ? '100%' : 'auto', gap: 1 }}>
      <TextField
        size="small"
        variant="outlined"
        placeholder={placeholder || "Buscar por código, status..."}
        value={inputValue}
